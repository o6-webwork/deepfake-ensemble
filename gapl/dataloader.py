...
class dataset_huggingface(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        print(f"repo {repo_id}, split{split}")
        self.split = split
        self.mode = mode
...
class SubsetWithTransform(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # x_0 = self.transform2(img)
        # x_minmin = self.transform2(x_minmin) 
        # x_maxmax = self.transform2(x_maxmax)

        # x_minmin1 = self.transform2(x_minmin1) 
        # x_maxmax1 = self.transform2(x_maxmax1)
        # return torch.stack([x_minmin, x_maxmax, x_minmin1, x_maxmax1, x_0], dim=0), lab, generator_name
        # Modified with AIDE
...
def get_train_dataloaders(
    ...
    if huggingface_repo_id != '':
        print(f"loading Dataset {huggingface_repo_id} now")
