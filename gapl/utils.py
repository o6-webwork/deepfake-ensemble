...
def get_logger(logfile="train.log"):
    """
    Get a logger instance with unbuffered output, and写入日志文件.
    """
    class UnbufferedStreamHandler(logging.StreamHandler):
        pass 

    logger = logging.getLogger("Generator_Aware_Prototype_Learning")
...
def parse_args():
    ...
    parser.add_argument("--stageone", action="store_true", help="Training on the first stage")
    parser.add_argument('--fe_path', type=str, default=None, help='Path of feature extractor')
    parser.add_argument('--prototype_path', type=str, help='Path of the extracted prototype')
    parser.add_argument('--compile', action='store_true', help='using torch.compile or not')
    # Model arguments
    ...
    if args.save_path == "":
        args.save_path = f"./trained_model/{time.strftime('%Y%m%d-%H%M%S')}{args.exp_name}/commfor_train.pt"
...
def train_one_epoch(
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        with torch.autocast(device_type='cuda', dtype=parse_floating_point_string(args.amp_dtype), enabled=args.use_amp):
            if args.stageone:
                ...
                if isinstance(criterion, nn.CrossEntropyLoss):
                    # CrossEntropyLoss expects class indices (no one-hot), shape [B]
                    lbl = labels.view(-1).to(device=device, dtype=torch.long)
                    taskloss = criterion(outputs, lbl)
                else:
                    # BCE-style losses expect float targets with same shape as outputs
        ...
        if args.verbose > 1 and rank == 0:
            try:
                # print(f'\r[Train] Epoch {epoch+1}/{args.epochs} | Iteration (this epoch) {i+1}/{len(train_loader)} |T Loss: {taskloss.item():.4f} |P Loss : {proto_loss:.4f} |B Loss : {balance_loss:.4f} |Local {local_loss:.4f} | Lat(Model/DL): {model_lat_pitr:.2f}/{dl_lat_pitr:.2f} s  ', end="", flush=True)
                print(f'\r[Train] Epoch {epoch+1}/{args.epochs} | Iteration (this epoch) {i+1}/{len(train_loader)} |T Loss: {taskloss.item():.4f} | Loss : {loss.item():.4f} |Local {local_loss:.4f} | Lat(Model/DL): {model_lat_pitr:.2f}/{dl_lat_pitr:.2f} s  ', end="", flush=True)
...
def evaluate_one_epoch(
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader, 0):
            ...
            if add_sigmoid and not args.stageone:
                outputs = torch.sigmoid(outputs)

            if separate_eval:
                # Only accumulate generator-level stats when generator_names is available
                if not args.stageone:
...
def multiclasseval(    
    ...
    device = model.device
    metric = tmc.MulticlassConfusionMatrix(num_classes=5).to(device)
    with torch.no_grad():
        for i, data in enumerate(dataloader, 0):
            inputs, labels = data
            inputs = inputs.to(device)
    ...
    m = metric.compute()
    tp = torch.diag(m).float()
    support = m.sum(dim=1).float().clamp(min=1.0)
    per_class_acc = (tp / support).cpu().tolist()
    for c, a in enumerate(per_class_acc):
        print(f"Class {c} acc={a:.4f} ({int(tp[c].item())}/{int(support[c].item())})")
...
def save_checkpoint(model, optimizer, scheduler, scaler, epoch, itr, ckpt_path):
    ...
    if hasattr(model, "module"):
        ...
        if hasattr(model.module, "proVec"):
            proVec = model.module.proVec
        else:
            proVec = None
    else:
...
def reorg_state_dict(state_dict, name):
    sd = dict(state_dict)
